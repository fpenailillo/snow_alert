...
def _ya_existe_pronostico_horas(
    cliente_bigquery: bigquery.Client,
    nombre_ubicacion: str,
    marca_ingestion: str
) -> bool:
    """Verifica si ya existe un pronóstico horario para esta ubicación en las últimas 2 horas."""
...
def procesar_pronostico_horas(evento_nube):
    ...
    try:
        ...
        if _ya_existe_pronostico_horas(cliente_bigquery, nombre_ubicacion, now_iso):
            logger.info(
                f"Pronóstico horario duplicado para {nombre_ubicacion} — omitiendo"
            )
            return
