"""
Snow Alert - Procesador de Pronóstico Diario

Cloud Function que procesa datos de pronóstico diario desde Pub/Sub,
los almacena en Cloud Storage (capa bronce) y BigQuery (capa plata).

Datos: Pronóstico de los próximos 5 días con temperaturas máx/mín,
condiciones diurnas/nocturnas, y métricas de precipitación.
...
def validar_datos_pronostico(datos: Dict[str, Any]) -> None:
    ...
    # Validar que exista forecastDays
    datos_raw = datos.get('datos_clima_raw', {})
    if 'forecastDays' not in datos_raw:
        raise ErrorValidacionDatos("Datos de pronóstico diario no encontrados (forecastDays)")
...
def construir_ruta_gcs(datos: Dict[str, Any]) -> str:
    ...
    try:
        ...
        ruta = (
            f"{nombre_ubicacion}/pronostico_dias/"
...
def extraer_datos_periodo(periodo: Dict[str, Any], prefijo: str, dia_data: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    datos = {
        ...
        f'{prefijo}_prob_precipitacion': extraer_valor_seguro(periodo, ['precipitation', 'probability', 'percent']),
        f'{prefijo}_cantidad_precipitacion': extraer_valor_seguro(periodo, ['precipitation', 'qpf', 'quantity']),
        f'{prefijo}_prob_tormenta': periodo.get('thunderstormProbability'),
...
def transformar_dia_para_bigquery(
    dia_data: Dict[str, Any],
    nombre_ubicacion: str,
    coordenadas: Dict[str, Any],
    marca_tiempo_extraccion: str,
    uri_gcs: str
) -> Dict[str, Any]:
    """Transforma un día de pronóstico al esquema de BigQuery."""

    # Extraer intervalo de tiempo
    intervalo = dia_data.get('interval', {})
    inicio_dia = intervalo.get('startTime', '')
    fin_dia = intervalo.get('endTime', '')

    # Extraer fecha de visualización
    display_date = dia_data.get('displayDate', {})
    ...
    fila = {
        ...
        'temp_min_dia': extraer_valor_seguro(dia_data, ['minTemperature', 'degrees']),
    }
    ...
    # Metadata
    fila['marca_tiempo_extraccion'] = marca_tiempo_extraccion
    fila['marca_tiempo_ingestion'] = datetime.now(timezone.utc).isoformat()
    fila['uri_datos_crudos'] = uri_gcs

    return fila


def transformar_datos_para_bigquery(datos: Dict[str, Any], uri_gcs: str) -> List[Dict[str, Any]]:
    """Transforma los datos de pronóstico diario a filas de BigQuery."""
    try:
        ...
        forecast_days = datos_clima_raw.get('forecastDays', [])

        filas = []
        for dia_data in forecast_days:
            fila = transformar_dia_para_bigquery(
                dia_data,
                nombre_ubicacion,
                coordenadas,
        ...
        logger.info(f"Transformados {len(filas)} días de pronóstico para {nombre_ubicacion}")
        return filas
...
def procesar_pronostico_dias(evento_nube):
    """
    Cloud Function que procesa mensajes de pronóstico diario desde Pub/Sub.

    Disparada por mensajes en el topic 'clima-pronostico-dias':
    1. Decodifica y valida el mensaje
    2. Guarda datos crudos en GCS (capa bronce)
    3. Transforma cada día al esquema de BigQuery
    4. Inserta filas en BigQuery (capa plata)
    """
    logger.info("=" * 60)
    logger.info("Procesando pronóstico diario")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"Procesando pronóstico diario para: {nombre_ubicacion}")
        ...
        logger.info("=" * 60)
        logger.info(f"Procesamiento completado: {nombre_ubicacion} ({len(filas_bigquery)} días)")
        logger.info("=" * 60)
